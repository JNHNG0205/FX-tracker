...
// currency->value map. e.g. {"rates":{"2026-06-09":{"MYR":4.061}}}.
type histResp struct {
	Rates map[string]map[string]float64 `json:"rates"`
}

// FetchHistory fetches the USD/MYR daily series in [start,end]. Business-day
...
func FetchHistory(ctx context.Context, client *http.Client, baseURL string, start, end time.Time) ([]HistPoint, error) {
	url := fmt.Sprintf("%s/%s..%s?from=USD&to=MYR",
		baseURL, start.Format("2006-01-02"), end.Format("2006-01-02"))
	...
	for dateStr, rates := range parsed.Rates {
		...
		pts = append(pts, HistPoint{Date: d, MyrUsd: 1.0 / myr})
	}
